...
### 第三步：互動式設定
安裝過程會詢問您：
- **安裝路徑**：預設會裝在 `%LOCALAPPDATA%\ContextTools`。您可以直接按 **Enter** 接受，或是輸入自訂路徑（如 `D:\Tools\ContextTools`）。
- **清理現場**：當畫面顯示「安裝成功」後，您就可以大方地將當初下載的原始資料夾**刪除**了。程式已經自動複製並安裝到您的系統路徑中，隨時待命。
